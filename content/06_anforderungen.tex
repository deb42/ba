...
In diesem Kapitel werden die bestehenden Anforderungen an ein E-Learning-Modul zum Assessment von B-Baum-Datenstrukturen vorgestellt und näher erläutert. Da es sich bei diesem E-Learning-Modul um den bereits existierenden EASy-DSBuilder handelt, welcher lediglich um die Funktion des Assessments von
B-Baum-Datenstrukturen erweitert werden muss, stellt das Kapitel \ref{sec:neuanforderungen} die Neuanforderungen an den EASy-DSBuilder vor. Anschließend wird in Kapitel \ref{sec:konzeptGrapheneditor} tiefgehender auf die Anforderungen des Grapheneditors eingegangen.
...
\paragraph{() Der Studierende soll einen Überblick über den aktuellen Aufgabenstand erhalten}
\begin{quote}
...
\paragraph{() Der Studierende soll die Syntax seiner Eingabe prüfen können}
\begin{quote}

\end{quote}

\paragraph{() Der Studierende soll seine Eingabe abgeben können}
\begin{quote}
...
\paragraph{() Der Studierende soll den aktuellen Schritt seiner Eingabe zurücksetzen können}
\begin{quote}

\end{quote}

\paragraph{() Der Studierende soll den vorherigen Schritt seiner Eingabe zurücksetzen können}
\begin{quote}

\end{quote}

\paragraph{() Es soll die Möglichkeit des Feedbacks gegeben werden}
\begin{quote}
...
\paragraph{() Navigationsmöglichkeiten innerhalb einer Aufgabe müssen vorhanden sein}
\begin{quote}
...
\paragraph{() Aufgabenschritte müssen auf der vorherigen Eingabe basieren}
\begin{quote}
...
\paragraph{() Der Lehrende soll eine Auswahlmöglichkeit erhalten, zwischen Datenstrukturen wählen zu können}
\begin{quote}
...
Jsdot bietet die Möglichkeit neben runden auch rechteckige Knoten zu erstellen. Aus diesen könnten Knoten generiert werden. Auf diese Weise könnten Studierende eine B-Baumstruktur auf der Basis dieser Knoten und Kanten erstellen, um darauf die jeweiligen Schlüsselelemente richtig zu positionieren. 

Jedoch ergeben sich daraus mehrere Probleme. Zum einen ist die Umsetzung der Positionierung der abwärtsgerichteten Kanten ein Problem. Idealtypisch ist die Positionierung der abwärtsgerichteten Kanten am Anfang oder am Ende eines Knotens oder zwischen zwei Schlüsseln (vergl. Abb. \ref{fig:bbaum}). 
